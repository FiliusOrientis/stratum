...
**File**: `apps/web/src/components/shared/fab.tsx`
...
- `Fab` — the component
- `FabPosition` — the position union type (reused by `ThemeToggle` `position` prop)
...
Composers: `ThemeToggle` (shared/theme-toggle.tsx) and `KeyboardShortcutsFab` (app-shell/keyboard-shortcuts-fab.tsx).
