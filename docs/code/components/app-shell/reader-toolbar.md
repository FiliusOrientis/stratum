...
| `reader-toolbar.types.ts`     | Shared types (`ToolbarButtonProps`, `ToolbarAnimation`) and pure animation math (`getAnimation()`)      |
...
- **Direction**: `getAnimation(isTop)` in `reader-toolbar.types.ts` derives the signed slide distances and edge (`top-0`/`bottom-0`)
