...
|--------|-----------|-------------|
| `setGeminiKey` | `(slot: number, key: string \| null) => void` | Set a key at a specific slot (0-9) |
| `clearGeminiKey` | `(slot: number) => void` | Clear a key slot |
...
```tsx
import { useSettingsStore, MAX_KEYS } from '@/stores'
